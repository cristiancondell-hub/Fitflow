...
const FITFLOW_CACHE_V5 = 'fitflow-prod-shell-2026-06-10-05';
  ...
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(
        keys
          .filter(key => key.startsWith('fitflow-prod-shell-') && key !== FITFLOW_CACHE_V5)
...
async function networkFirstNavigation(request) {
  const cache = await caches.open(FITFLOW_CACHE_V5);
  try {
