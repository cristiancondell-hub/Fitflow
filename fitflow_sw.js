...
const FITFLOW_CACHE_V4 = 'fitflow-prod-shell-2026-06-09-04';
  ...
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(
        keys
          .filter(key => key.startsWith('fitflow-prod-shell-') && key !== FITFLOW_CACHE_V4)
...
async function networkFirstNavigation(request) {
  const cache = await caches.open(FITFLOW_CACHE_V4);
  try {
